...
import { Accordain_Player } from "./Accordian_Player";
import Player_Policy from "./Player_Policy";

const Help_And_Support = () => {
  const [selected, setSelected] = useState("FAQ");

  return (
    <div className="p-3 w-[75%] absolute top-20 right-0 mx-auto bg-[#fafafa] max-lg:top-32 max-lg:w-[100%] max-lg:mt-10">
      <div className="w-full px-5 rounded-lg shadow-md bg-white py-5 gap-3 flex flex-col">
        ...
        <hr />
        {selected === "FAQ" && <Accordain_Player />}
        {selected === "PrivacyPolicy" && <Player_Policy />}
      </div>
    </div>
  );
};
